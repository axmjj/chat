...
### 第三阶段：WebSocket 通信核心
- [x] **连接管理器 (`app/connection.py`)**
    - [x] 实现 `ConnectionManager` 类
    - [x] 管理活跃连接池 (`active_connections`)
    - [x] 实现点对点发送 (`send_personal_message`)
    - [x] 实现广播发送 (`broadcast`)
- [x] **WebSocket 路由 (`app/routers/chat.py`)**
    - [x] 定义 WebSocket 端点 (`/ws`)
    - [x] WebSocket 鉴权 (Token 验证)
    - [x] 消息分发逻辑 (解析 JSON -> 存储 -> 转发)
...
- [x] **项目初始化**
    ...
    - [x] 安装依赖 (`axios`, `react-router-dom`, `zustand`)
- [x] **路由配置**
    - [x] 配置 React Router (`/login`, `/register`, `/chat`)
- [x] **认证页面**
    - [x] 开发登录页面 (Login Page)
    - [x] 开发注册页面 (Register Page)
    - [x] 实现 API 请求服务 (`services/api.ts`)

### 第二阶段：聊天界面布局
- [x] **主布局 (Layout)**
    - [x] 侧边栏 (Sidebar): 用户头像、导航
    - [x] 联系人列表 (ContactList): 显示所有用户/在线用户
    - [x] 聊天主窗口 (ChatWindow): 顶部栏、消息区域、输入框
- [x] **UI 组件**
    - [x] `ChatSidebar`: 侧边栏组件
    - [x] `ChatWindow`: 聊天窗口组件
...
- [x] **状态管理 (Store)**
    - [x] `useAuthStore`: 存储用户信息、Token
    - [x] `useChatStore`: 存储当前聊天对象、消息列表、在线用户
- [x] **WebSocket 服务 (`services/websocket.ts`)**
    - [x] 封装 WebSocket 连接、断开、重连逻辑
    - [x] 监听 `onmessage` 并更新 Store
- [x] **业务逻辑联调**
    - [x] 发送消息 -> 更新本地 UI -> 发送 WS 请求
    - [x] 接收消息 -> 判断来源 -> 更新对应会话的消息列表
